...
class LaserDelegate(btle.DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        ...
        print_response(hex_values)  # Process the received hex values
...
def print_response(hex_values, insert_type):
    ...
    if formatted_hex[0] == "c0" and formatted_hex[1] == "55" and formatted_hex[2] == "10" and formatted_hex[3] == "06":
        ...
        pyperclip.copy(str(measurement_value_cm_rounded))  # Copy the measurement to the clipboard
        print(f"Measured length: {measurement_value_cm_rounded} (copied to clipboard)")

        # Insert the value at the current cursor position
        time.sleep(0.1)  # Wait to ensure the clipboard is ready
...
def main():
    ...
    if not mac_address:
        if os.path.exists("bosch-bluetoothmac.txt"):
            with open("bosch-bluetoothmac.txt", "r") as f:
    ...
    # Set the delegate for the laser
    device.setDelegate(LaserDelegate())
